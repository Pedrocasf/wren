...
static void getSlots(WrenVM* vm)
{
  ...
  if (wrenGetSlotFloat(vm, 3) != 1.5) result = false;
...
static void ensureOutsideForeign(WrenVM* vm)
{
  ...
  for (int i = 0; i < 20; i++)
  {
    sum += (int)wrenGetSlotFloat(otherVM, i);
  }
...
{
  wrenSetSlotFloat(vm, 0, wrenGetListCount(vm, 1));
}
