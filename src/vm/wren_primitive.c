...
static uint32_t validateIndexValue(WrenVM* vm, uint32_t count, float value,
                                   const char* argName)
{
...
bool validateIntValue(WrenVM* vm, float value, const char* argName)
{
                        ...
                        int* step)
{
  *step = 0;
  ...
  if (range->from == *length &&
      range->to == (range->isInclusive ? -1.0 : (float)*length))
  {
    *length = 0;
    return 0;
  }
